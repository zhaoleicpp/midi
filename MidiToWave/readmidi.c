...
#include "strtab.h"
#include "memb.h"
//#include "zip.h" //Zhao Lei deleted
//#include "arc.h" //Zhao Lei deleted
//#include "mod.h" //Zhao Lei deleted
#include "wrd.h"
#include "tables.h"
#include "reverb.h"
#include <math.h>
...
const struct ctl_chg_types ctl_chg_list[] ={
      { 0, ME_TONE_BANK_MSB },
...
static int first__readmidi_read_init = 1;

void readmidi_read_init(void)
{
    int i;
	int *first = &first__readmidi_read_init;
#define first (*first)
...
void Intialize_readmidi()
{
	...
	play_system_mode = DEFAULT_SYSTEM_MODE;
	evlist=NULL;
	current_midi_point=NULL;
	memset(&string_event_strtab,0,sizeof(string_event_strtab));
	midi_file_info = NULL;
	string_event_table = NULL;
	string_event_table_size = 0;
	time_segments = NULL;
	userdrum_first = (UserDrumset *)NULL;
	userdrum_last = (UserDrumset *)NULL;
	userinst_first = (UserInstrument *)NULL;
	userinst_last = (UserInstrument *)NULL;

	memset(rhythm_part,0,sizeof(rhythm_part));
	event_count=0;
	sample_increment=0;
	memset(timesig,0,sizeof(timesig));
	sample_correction=0;
	first__readmidi_read_init = 1;
	//_ignore_midi_error file_name=readmidi length=72
	//_userinst_last file_name=readmidi length=313620
}
