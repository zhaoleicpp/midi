...
#include "miditrace.h"
#include "recache.h"
//#include "arc.h" //Zhao Lei deleted Ã»ÓÐÑ¹Ëõ½âÑ¹
#include "reverb.h"
#include "wrd.h"
#include "aq.h"
#include "freq.h"
...
static int old_voices__restore_voices = -1;
void restore_voices(int save_voices)
{
#ifdef REDUCE_VOICE_TIME_TUNING
	int *old_voices = &old_voices__restore_voices;
...
static int play_count__play_midi = 0;

static int play_midi(MidiEvent *eventlist, int32 samples)
{
    int rc;
	int *play_count = &play_count__play_midi;
#define play_count (*play_count)
...
static int last_rc__play_midi_file = RC_NONE;

int play_midi_file(char *fn)
{
    int i, j, rc;
    int *last_rc = &last_rc__play_midi_file;
#define last_rc (*last_rc)
    MidiEvent *event;
    int32 nsamples;
...
static int last_secs__ctl_timestamp = -1;
static int last_voices__ctl_timestamp = -1;
static void ctl_timestamp(void)
{
    long i, secs, voices;
    CtlEvent ce;
    int *last_secs = &last_secs__ctl_timestamp;
	int *last_voices = &last_voices__ctl_timestamp;
#define last_secs (*last_secs)
#define last_voices (*last_voices)
...
void Intialize_playmidi()
{
	...
	opt_tva_attack = 0;	/* attack envelope control */
	opt_tva_decay = 0;	/* decay envelope control */
	opt_tva_release = 0;	/* release envelope control */
	opt_delay_control = 0;	/* CC#94 delay(celeste) effect control */
	opt_eq_control = 0;		/* channel equalizer control */
	opt_insertion_effect = 0;	/* insertion effect control */
	opt_drum_effect = 0;	/* drumpart effect control */
	opt_drum_power = 100;		/* coef. of drum amplitude */
	opt_amp_compensation = 0;
	opt_modulation_envelope = 0;
	opt_pan_delay = 0;	/* phase difference between left ear and right ear. */
	opt_user_volume_curve = 0;
	opt_default_module = MODULE_TIMIDITY_DEFAULT;
	opt_preserve_silence = 0;
	voices=DEFAULT_VOICES;
	control_ratio=0,
		amplification=DEFAULT_AMPLIFICATION;
	master_volume_ratio = 0xFFFF;
	adjust_panning_immediately = 1;
	auto_reduce_polyphony = 1;
	envelope_modify_rate = 1.0;
	reduce_quality_flag = 0;
	no_4point_interpolation = 0;
	pcm_alternate_file = NULL;
	reverb_buffer = NULL; /* MAX_CHANNELS*AUDIO_BUFFER_SIZE*8 */
	note_key_offset = 0;		/* For key up/down */
	midi_time_ratio = 1.0;	/* For speed up/down */
	compensation_ratio = 1.0; /* compensation ratio */
	master_volume=0;
	...
	sample_count=0;
	mainvolume_max=0;
	//_envelope_modify_rate
	//_pcm_alternate_file
	old_voices__restore_voices = -1;
	last_secs__ctl_timestamp = -1;
	last_voices__ctl_timestamp = -1;
	play_count__play_midi = 0;
	last_rc__play_midi_file = RC_NONE;
}
