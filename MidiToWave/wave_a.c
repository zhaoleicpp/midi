...
const static char *orig_RIFFheader=
...
static intptr_t wav_output_open(const char *fname)
{
  int t;
  char RIFFheader[44];
  intptr_t fd;
  ...
  if(strcmp(fname, "-") == 0)
    ...
    /* Open the audio file */
    fd = output_open(fname, FILE_OUTPUT_MODE);
    if(fd < 0) {
  ...
  *((int *)(RIFFheader+28)) = LE_LONG(t);
  ...
  if(output_std_write(fd, RIFFheader, 44) == -1) {
...
static intptr_t auto_wav_output_open(const char *input_filename)
{
  ...
  if((dpm._fd = wav_output_open(output_filename)) == -1) {
...
static int open_output(void)
{
    ...
    if(dpm.name == NULL) {
      ...
      dpm.flag &= ~PF_AUTO_SPLIT_FILE;
      if((dpm._fd = wav_output_open(dpm.name)) == -1)
	return -1;
    }
#else
...
{
    //off_t save_point;
    int save_point;//Zhao Lei modifired
    int32 tmp;

    if(already_warning_lseek)
	return 0;

    save_point = output_lseek(dpm._fd, 0, SEEK_CUR);
    if(save_point == -1 || output_lseek(dpm._fd, 4, SEEK_SET) == -1)
    {
    ...
    if(output_std_write(dpm._fd, (char *)&tmp, 4) == -1)
    {
    output_lseek(dpm._fd, save_point, SEEK_SET);
	return -1;
    }
    output_lseek(dpm._fd, 40, SEEK_SET);
    tmp = LE_LONG(bytes_output);
    output_std_write(dpm._fd, (char *)&tmp, 4);
...
static void close_output(void)
{
    if(dpm._fd != 1 && /* We don't close stdout */
       dpm._fd != -1)
    {
	update_header();
