...
//#include <sys/stat.h> //Zhao Lei deleted
...
#include "output.h"
#include "controls.h"
//#include "arc.h"//Zhao Lei Joy deleted 压缩用的，这里用不到
//#include "nkflib.h"//Zhao Lei 
#include "wrd.h"
#include "strtab.h"
#include "support.h"
...
#ifdef DEFAULT_PATH
	/* The paths in this list will be tried whenever we're reading a file */
	defaultpathlist.path=DEFAULT_PATH;
	defaultpathlist.next=0;
	pathlist=&defaultpathlist; /* This is a linked list */
#else
	pathlist=0;
#endif


	//CGlobalVariableInitialValueCheck::Check var name=_output_text_code file_name=common length=39720
...
{
	//Zhao Lei Joy deleted, it never be compressed
	//根本没有什么压缩文件，走到这里直接返回输入文件
	return NULL;
	/*
...
long tf_tell(struct timidity_file *tf)
{
    ...
    {
		//Zhao Lei modified 
		//It never fail to get positon
		//ctl->cmsg(CMSG_WARNING, VERB_NORMAL,
		//	  "Warning: Can't get current file position");
		//return (long)tf->url->nread;
		ctl->cmsg(CMSG_ERROR,VERB_NORMAL,
			"Error: Can't get current file position");
		return 0;
    }
