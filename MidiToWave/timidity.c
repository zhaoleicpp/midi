...
#include "resample.h"
#include "recache.h"
//#include "arc.h" //Zhao Lei deleted
#include "strtab.h"
#include "wrd.h"
...
#include "aq.h"
#include "mix.h"
//#include "unimod.h"//mod 文件格式，不需要
#include "quantity.h"
//#include "rtsyn.h"//好像是驱动
...
static int is_first__timidity_start_initialize = 1;

MAIN_INTERFACE void timidity_start_initialize(void)
{
    int i;
    static int drums[] = DEFAULT_DRUMCHANNELS;
	int *is_first = &is_first__timidity_start_initialize;
#define is_first (*is_first)
    ...
    //uudecode_unquote_html = 1; //Zhao Lei
    for(i = 0; i < MAX_CHANNELS; i++)
    {
	default_program[i] = DEFAULT_PROGRAM;
	memset(channel[i].drums, 0, sizeof(channel[i].drums));
    }
    //arc_error_handler = timidity_arc_error_handler; //Zhao Lei
...
MAIN_INTERFACE int timidity_post_load_configuration(void)
{
	...
	if(config_string_list != NULL)
	{
		...
		config_string_list[0]=NULL;
	    free(config_string_list);
		config_string_list=NULL;
	}
    }
...
//[
void Intialize_mt19937ar();
void Intialize_getopt();
void Intialize_version();
void Intialize_wrd_read();
void Intialize_tables();
void Intialize_optcode();
void Intialize_mblock();
void Intialize_mix();
void Intialize_recache();
void Intialize_freq();
void Intialize_effect();
void Intialize_aq();
void Intialize_dumb_c();
void Intialize_playmidi();
void Intialize_output();
void Intialize_controls();
void Intialize_sndfont();
void Intialize_wrdt();
void Intialize_common();
void Intialize_readmidi();
void Intialize_resample();
void Intialize_instrum();
void Intialize_timidity();
void Intialize_wave_a();
void Intialize_raw_a();
void Intialize_reverb();
void Intialize_sffile();
void Intialize_list_a();
//void g_global_variable_initial_value_check_Go();//debug
//]
...
#include <sys/resource.h>
	...
	//[
	{
		...
		Intialize_sffile();
		Intialize_list_a();
		//g_global_variable_initial_value_check_Go();//debug
	}
	//]
	//memset(tonebank,0,sizeof(tonebank));
	//memset(drumset,0,sizeof(drumset));
	memset(tonebank,0,sizeof(ToneBank *)*(128 + MAP_BANK_COUNT));
	memset(drumset,0,sizeof(ToneBank *)*(128 + MAP_BANK_COUNT));
	tonebank[0] =&standard_tonebank,
	drumset[0] =&standard_drumset;
...
/* Try to continue if it is Windows version */
	...
	if (nfiles > 0
			...
			&& ctl->id_character != 'P') {
		//1谁申请谁释放，根本不是这个函数申请的
		//2原代码由expand_file_archives申请，现在这个函数不申请了
		//free(files_nbuf);
		//free(files);
	}
	free_soft_queue();
...
void Intialize_timidity()
{
	...
	opt_control_ratio = 0; /* Save -C option */
#ifdef IA_DYNAMIC
	static char *dynamic_lib_root = NULL;
#endif
	free_instruments_afterwards=0;
	def_prog = -1;
	memset(def_instr_name,0,sizeof(def_instr_name));
	intr = 0;
	effect_lr_mode = -1;
	effect_lr_delay_msec = 25;
	opt_buffer_fragments = -1;

	got_a_configuration=0;
	opt_output_rate=0;
	is_first__timidity_start_initialize = 1;
}
